...
def add_resolver():
    ...
    try:
        ...
        if existing_user:
            return jsonify({
                'success': False,
                'message': f'This email belongs to user "{existing_user.full_name}". A resolver cannot use a user\'s email.'
            }), 400
        ...
        if existing_admin:
            return jsonify({
                'success': False,
                'message': f'This email belongs to admin "{existing_admin.full_name}". A resolver cannot use an admin\'s email.'
            }), 400
